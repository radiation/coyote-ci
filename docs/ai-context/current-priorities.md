...
- the current notification delivery refactor replaces recipient-string dedupe with transport-aware logical delivery identity backed by stable internal destination ids; acquisition is now atomic, current behavior still suppresses any existing sent/pending/failed row, and bounded retry plus stale-delivery recovery remain deferred to the next notification slice
...
- Bounded retry and stale-claim recovery for notification deliveries are the next likely backend follow-on after the logical-delivery identity refactor.
