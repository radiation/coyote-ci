...
- `backend/internal/service/build/notifications.go`: terminal build email, shared Slack webhook, and personal Slack DM formatting/delivery for failed and successful builds, notification target/subscription resolution, commit-author personal preference evaluation, env email fallback for local/default setups, transport-aware logical delivery planning keyed by stable internal destination ids, duplicate-suppressing best-effort delivery acquisition, and the dev-only sample email send path.
