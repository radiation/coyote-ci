...
- `backend/internal/platform/slack/client.go`: Slack API client for `auth.test` workspace verification and live `users.lookupByEmail` member resolution.
- `backend/internal/service/build/notifications.go`: terminal build email and Slack webhook formatting/delivery for failed and successful builds, notification target/subscription resolution, env email fallback for local/default setups, durable per-destination delivery tracking/dedupe, and the dev-only sample email send path.
...
- `backend/internal/service/notification_service.go`: admin-style notification target/subscription validation and CRUD orchestration for backend API management of email and Slack webhook targets plus scoped subscriptions.
...
- Build notifications: start with `backend/internal/platform/config/config.go`, `backend/internal/platform/email/sender.go`, `backend/internal/service/build/notifications.go`, `backend/internal/service/build/slack_sender.go`, `backend/internal/service/notification_service.go`, `backend/internal/service/build/lifecycle.go`, `backend/internal/service/build/completion.go`, `backend/internal/service/build/source.go`, `backend/internal/repository/notification_delivery_repository.go`, `backend/internal/repository/notification_subscription_repository.go`, `backend/db/migrations`, `backend/cmd/server/main.go`, `backend/cmd/worker/main.go`, `backend/internal/http/handler/notification_handler.go`, and `backend/internal/http/router.go` for both admin notification management endpoints and the dev-only sample route.
- Personal Slack identity linking: start with `backend/internal/service/user_slack_identity_service.go`, `backend/internal/http/handler/notification_personal_slack_identity_handler.go`, `backend/internal/api/notification_dto.go`, `backend/internal/platform/slack/client.go`, `backend/internal/repository/user_slack_identity_repository.go`, `backend/internal/repository/memory/slack_workspace_integration_repository.go`, `backend/internal/repository/postgres/slack_workspace_integration_repository.go`, `backend/internal/repository/memory/user_slack_identity_repository.go`, `backend/internal/repository/postgres/user_slack_identity_repository.go`, `backend/db/migrations/00030_add_user_slack_identities.sql`, and the matching `*_test.go` files.
