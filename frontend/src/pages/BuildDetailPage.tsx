  ...
  return (
    <>
      ...
      <div className="detail-summary">
        <span>
          <strong>Project:</strong> {build.project_id}
        </span>
        <span>
          <strong>Status:</strong> <StatusBadge status={build.status} />
        </span>
        <span>
          <strong>Current Step:</strong> {build.current_step_index}
        </span>
        <span>
      ...
      <p className="subtle-text">
        {compactTriggerMetadata(build) ||
          "No trigger metadata available for this build."}
      </p>
      <p className="subtle-text">
        Last updated:{" "}
        {buildUpdatedAt > 0
      ...
      <div className="detail-grid">
        ...
        <div>
          ...
          <span>{formatTime(build.queued_at)}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span>{formatTime(build.finished_at)}</span>
        </div>
        {build.pipeline_source && (
          <div>
            <strong>Pipeline Source</strong>
            <span>{build.pipeline_source}</span>
          </div>
        )}
        {build.pipeline_path && (
          <div>
            <strong>Pipeline Path</strong>
            <span>{build.pipeline_path}</span>
          </div>
        )}
        {build.scm_provider && (
          <div>
            <strong>SCM Provider</strong>
            <span>{build.scm_provider}</span>
          </div>
        )}
        {build.event_type && (
          <div>
            <strong>Event Type</strong>
            <span>{build.event_type}</span>
          </div>
        )}
        {build.trigger_ref && (
          <div>
            <strong>Trigger Ref</strong>
        ...
        build.trigger_commit_sha !== build.source_commit_sha ? (
          <>
            <div>
              <strong>Trigger Commit</strong>
              <span>{shortSHA(build.trigger_commit_sha)}</span>
            </div>
            <div>
              <strong>Source Commit</strong>
              <span>{shortSHA(build.source_commit_sha)}</span>
            </div>
          </>
        ) : (
          <div>
        ...
        <div>
          <strong>Error</strong>
          <span className="error-text">{build.error_message ?? "—"}</span>
        </div>
      </div>
