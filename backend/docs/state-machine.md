...
- When `COYOTE_WORKSPACE_REVISION_STORAGE_ROOT` is configured, a successful command publishes immutable workspace bytes, marks its revision published under the active claim, and atomically exposes successful step and execution-job state. Without that setting, Docker/local execution retains its legacy local-only workspace behavior. Restore is not yet wired into runtime execution.
