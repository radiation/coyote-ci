...
func TestArtifactRepository_Create(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO build_artifacts").
		WithArgs("artifact-1", "build-1", &stepID, "dist/output.zip", "generic", "build-1/dist/output.zip", "filesystem", int64(10), &contentType, &checksum, now).
		WillReturnRows(sqlmock.NewRows([]string{"id", "build_id", "step_id", "logical_path", "artifact_type", "storage_key", "storage_provider", "size_bytes", "content_type", "checksum_sha256", "created_at"}).
			AddRow("artifact-1", "build-1", &stepID, "dist/output.zip", "generic", "build-1/dist/output.zip", "filesystem", int64(10), contentType, checksum, now))
...
func TestArtifactRepository_Create_ZeroCreatedAtUsesDatabaseNow(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO build_artifacts").
		WithArgs("artifact-1", "build-1", nil, "dist/output.zip", nil, "build-1/dist/output.zip", "filesystem", int64(10), nil, nil, nil).
		WillReturnRows(sqlmock.NewRows([]string{"id", "build_id", "step_id", "logical_path", "artifact_type", "storage_key", "storage_provider", "size_bytes", "content_type", "checksum_sha256", "created_at"}).
			AddRow("artifact-1", "build-1", nil, "dist/output.zip", nil, "build-1/dist/output.zip", "filesystem", int64(10), nil, nil, now))
...
func TestArtifactRepository_GetByID_NotFound(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, build_id, step_id, logical_path, artifact_type, storage_key, storage_provider, size_bytes, content_type, checksum_sha256, created_at").
...
func TestArtifactRepository_ListForBrowse(t *testing.T) {
	...
	buildRows := sqlmock.NewRows([]string{
		"id", "build_id", "step_id", "logical_path", "artifact_type", "storage_key", "storage_provider", "size_bytes", "content_type", "checksum_sha256", "created_at",
		...
		"artifact-1", "build-1", "step-1", "packages/pkg-a.tgz", "npm_package", "build-1/pkg-a.tgz", "filesystem", int64(12), "application/gzip", "abc123", now,
	...
	mock.ExpectQuery(regexp.QuoteMeta(`
		SELECT 
			a.id, a.build_id, a.step_id, a.logical_path, a.artifact_type, a.storage_key, a.storage_provider, a.size_bytes, a.content_type, a.checksum_sha256, a.created_at,
