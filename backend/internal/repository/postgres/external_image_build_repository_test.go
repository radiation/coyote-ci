...
func externalImageBuildRow(now time.Time) []driver.Value {
	return []driver.Value{"execution-1", "cloud_build", "intent", "build-1", "resource-1", "sources", "source.tar.gz", "1", []byte("[]"), "coyote-ci/backend", nil, "running", nil, nil, "https://logs", nil, now, now}
}
