...
func TestExecutionJobOutputRepository_CreateMany_RollsBackOnExecError(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create sql mock: %v", err)
	}
	...
	mock.ExpectBegin()
	mock.ExpectExec("INSERT INTO build_job_outputs").WillReturnError(errors.New("insert failed"))
	mock.ExpectRollback()
	...
	if err == nil {
		t.Fatal("expected create many to fail on insert error")
	}
