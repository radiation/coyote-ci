...
func TestNotificationDeliveryClaimOutcomeFromExisting(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "active claim maps to claimed by other", delivery: domain.NotificationDelivery{Status: domain.NotificationDeliveryStatusSending, ClaimExpiresAt: &expiresLater}, want: NotificationDeliveryClaimOutcomeClaimedByOther},
		{name: "expired claim maps to stale claim reclaimed", delivery: domain.NotificationDelivery{Status: domain.NotificationDeliveryStatusSending, ClaimedAt: &claimedAt, ClaimExpiresAt: &expiresEarlier}, want: NotificationDeliveryClaimOutcomeStaleClaimReclaimed},
