...
func TestParsePullRequestEvent_AcceptsSupportedSameRepositoryActions(t *testing.T) {
	for _, action := range []string{"opened", "reopened", "synchronize"} {
		t.Run(action, func(t *testing.T) {
			...
			body := []byte(`{"action":"` + action + `","installation":{"id":999},"repository":{"id":1001,"name":"backend","html_url":"https://github.com/example/backend","owner":{"login":"example"}},"pull_request":{"head":{"ref":"feature/pr","sha":"head-sha","repo":{"id":1001}}},"sender":{"login":"octocat"}}`)

			event, err := ParsePullRequestEvent(headers, body)
			if err != nil {
				t.Fatalf("parse event: %v", err)
			}
			if !event.SupportedAction || !event.SameRepository || event.Ref != "feature/pr" || event.CommitSHA != "head-sha" {
...
func TestParsePullRequestEvent_RejectsMalformedSupportedPayloads(t *testing.T) {
	...
	for _, body := range [][]byte{
		[]byte(`{`),
		[]byte(`{"action":"opened","installation":{"id":999},"repository":{"id":"1001","name":"backend","owner":{"login":"example"}},"pull_request":{"head":{"ref":"feature","sha":"head","repo":{"id":1001}}}}`),
		[]byte(`{"action":"opened","installation":{"id":999},"repository":{"id":1001,"name":"backend","owner":{"login":"example"}},"pull_request":{"head":{"ref":"","sha":"head","repo":{"id":1001}}}}`),
		[]byte(`{"action":"opened","installation":{"id":999},"repository":{"id":1001,"name":"backend","owner":{"login":"example"}},"pull_request":{"head":{"ref":"feature","sha":"","repo":{"id":1001}}}}`),
	} {
