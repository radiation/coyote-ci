...
func NewWorkspacePublishService(config WorkspacePublishServiceConfig) (*WorkspacePublishService, error) {
	...
	return &WorkspacePublishService{capabilities: config.CapabilityAuthorizer, executionJobs: config.ExecutionJobs, revisions: config.WorkspaceRevisions, store: config.RevisionStore, maxUploadBytes: maxUploadBytes}, nil
}
...
func (s *WorkspacePublishService) Publish(ctx context.Context, capabilityToken string, executionJobID string, podUID string, archive io.Reader) (domain.WorkspaceRevision, error) {
	_, authorizeErr := s.capabilities.Authorize(ctx, capabilityToken, executionJobID, podUID, domain.WorkspaceHelperRolePublish)
	if authorizeErr != nil {
	...
	restoreErr := workspace.RestoreArchive(ctx, archiveFile, publication, restoredRoot)
	...
	durablePublication, publishErr := s.store.Publish(ctx, revisionID, restoredRoot)
	if publishErr != nil {
		return domain.WorkspaceRevision{}, fmt.Errorf("publishing workspace revision: %w", publishErr)
	}
	published, markErr := s.revisions.MarkPublishedIfClaimed(ctx, revisionID, *job.ClaimToken, durablePublication, time.Now().UTC())
	if markErr != nil {
