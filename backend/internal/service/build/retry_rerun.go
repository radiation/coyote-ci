...
func buildAttemptFromSource(source domain.Build, now time.Time, rerunFrom *int) domain.Build {
	...
	build := domain.Build{
		...
		RegisteredRepositoryID: cloneStringPtr(source.RegisteredRepositoryID),
		SCMConnectionID:        cloneStringPtr(source.SCMConnectionID),
		ProviderRepositoryID:   cloneStringPtr(source.ProviderRepositoryID),
	}
