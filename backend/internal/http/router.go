...
func NewRouter(buildHandler *handler.BuildHandler, artifactHandler *handler.ArtifactHandler, jobHandler *handler.JobHandler, projectHandler *handler.ProjectHandler, versionTagHandler *handler.VersionTagHandler, credentialHandler *handler.SourceCredentialHandler, eventHandler *handler.EventHandler, pushEventSecret string, options ...RouterOption) nethttp.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		if cfg.workspaceHelperHandler != nil {
			...
			r.Post("/internal/workspace-helper/cache/publish-claim", cfg.workspaceHelperHandler.ClaimCachePublish)
			r.Post("/internal/workspace-helper/cache/publish-claim/release", cfg.workspaceHelperHandler.ReleaseCachePublishClaim)
		...
		r.Group(func(r chi.Router) {
			...
			if cfg.notificationHandler != nil {
				...
				r.Get("/me/notification-preferences/commit-author-successes", cfg.notificationHandler.GetMyCommitAuthorSuccessNotificationPreference)
				...
				r.Route("/notification-targets", func(r chi.Router) {
					...
					r.Delete("/{targetID}", cfg.notificationHandler.DeleteTarget)
				})
